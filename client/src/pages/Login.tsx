...
export default function Login() {
  ...
  const usingOAuth = Boolean(import.meta.env.VITE_OAUTH_PORTAL_URL && import.meta.env.VITE_APP_ID);
  ...
  const continueWithGoogle = () => {
    if (usingOAuth) {
      window.location.href = getAuthUrl(mode, "google");
      return;
    }

    toast.info("Google ile giriş, canlı ortamda OAuth bağlandığında aktif olacak.");
  };
            ...
            <div className="mx-auto grid max-w-6xl items-center gap-8 lg:grid-cols-[1fr_440px]">
              <div className="max-w-2xl">
                ...
                <h1 className="text-4xl font-bold tracking-tight md:text-6xl">
                  llinktr hesabınıza
                  <br />
                  <span className="text-primary">{mode === "signIn" ? "giriş yapın." : "kayıt olun."}</span>
                </h1>
                <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground md:text-lg">
                  Bio sayfalarınızı oluşturun, linklerinizi kısaltın, QR kodlarınızı yönetin ve tüm içeriğinizi tek yerden yayınlayın.
                </p>

                <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-border/50 bg-card/80 p-4">
                    <p className="text-xs text-muted-foreground">Anında yayın</p>
                    <p className="mt-1 text-sm font-semibold">Tema ve bloklar canlı güncellenir</p>
                  </div>
                  <div className="rounded-2xl border border-border/50 bg-card/80 p-4">
                    <p className="text-xs text-muted-foreground">Tek panel</p>
                    <p className="mt-1 text-sm font-semibold">Bio, kısa link ve QR tek akışta</p>
                  </div>
                  <div className="rounded-2xl border border-border/50 bg-card/80 p-4">
                    <p className="text-xs text-muted-foreground">Hızlı erişim</p>
                    <p className="mt-1 text-sm font-semibold">Telefon ve masaüstü önizleme hazır</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[1.8rem] border border-border/50 bg-card/90 p-6 shadow-2xl backdrop-blur">
                ...
                <div className="space-y-4">
                  <div>
                    <h2 className="text-2xl font-semibold">{mode === "signIn" ? "Hesabınıza giriş yapın" : "Yeni hesap oluşturun"}</h2>
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                      {mode === "signIn"
                        ? "E-posta adresiniz ve şifrenizle giriş yapın. Google oturumu canlı ortamda OAuth ile çalışır."
                        : "Dakikalar içinde hesabınızı oluşturup panelinizi kullanmaya başlayın."}
                    </p>
                  </div>
                  ...
                  {usingOAuth ? (
                    <a href={getAuthUrl(mode, "email")} className="block">
                      <Button size="lg" className="w-full bg-primary font-semibold text-primary-foreground hover:bg-primary/90">
                        {mode === "signIn" ? "E-posta ile giriş akışını başlat" : "E-posta ile kayıt akışını başlat"}
