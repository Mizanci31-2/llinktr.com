...
const QR_TYPES: { id: QRType; label: string; placeholder: string; icon: string }[] = [
  ...
  { id: "email", label: "E-posta", placeholder: "ornek@email.com", icon: "	" },
  { id: "text", label: "Metin", placeholder: "QR kodunuzun i�erii...", icon: "=�" },
...
export default function QRGenerator() {
  ...
  const generateQR = async (content: string) => {
    ...
    try {
      ...
      toast.error("QR kod olu_turulamad1");
    } finally {
      ...
      <div className="flex-1 container py-12">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-bold mb-3">QR Kod Olu_turucu</h1>
            <p className="text-muted-foreground">URL, telefon, e-posta, metin ve WhatsApp i�in QR kodlar olu_turun.</p>
          </div>
              ...
              <div className="p-5 rounded-2xl bg-card border border-border/50 space-y-4">
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-wider text-muted-foreground">
                    {currentType.label} {qrType === "whatsapp" ? "Numaras1" : qrType === "phone" ? "Numaras1" : ""}
                  </Label>
                  {qrType === "text" ? (
                    <Textarea
                      value={value}
                ...
                {value && (
                  <div className="p-3 rounded-lg bg-muted/50 border border-border/30">
                    <p className="text-xs text-muted-foreground mb-1 font-medium">QR 0�erii:</p>
            ...
            <div className="flex flex-col items-center">
              <div className="sticky top-24 w-full">
                <div className="p-6 rounded-2xl bg-card border border-border/50 flex flex-col items-center">
                  <p className="text-xs uppercase tracking-wider text-muted-foreground mb-4 font-semibold">�nizleme</p>
                  ...
                  <div className="w-[220px] h-[220px] rounded-xl bg-white flex items-center justify-center border border-border/30 overflow-hidden">
                    {isGenerating ? (
                      ...
                      <div className="text-center p-4">
                        ...
                        <p className="text-xs text-gray-400">0�erik girin</p>
                      </div>
                    )}
                  </div>
