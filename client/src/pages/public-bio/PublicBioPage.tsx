...
function getDividerColor(data: Record<string, string | boolean | number> | null, fallback: string) {
...
const LOCATION_MARKER_COLORS = ["#D6FF00", "#EF4444", "#38BDF8", "#A78BFA", "#FB923C", "#FACC15"];

function getMarkerColor(value?: string | number | boolean) {
  const color = String(value || "").trim();
  return LOCATION_MARKER_COLORS.some((item) => item.toLowerCase() === color.toLowerCase()) ? color : "#D6FF00";
}

function createLocationEntry(patch: Partial<LocationEntry> = {}): LocationEntry {
  return {
    id: patch.id || "location",
...
function getMapState(entries: LocationEntry[]) {
  ...
  return { points, centerWorld, zoom, viewport };
}
...
function LocationMapView({
  ...
  const mapState = getMapState(entries);
  if (mapState) {
    const tileSize = 256;
    ...
    return (
      <div className="relative h-[220px] overflow-hidden bg-[#101418] md:h-[240px]">
        <div className="absolute left-1/2 top-0 h-[220px] w-[360px] -translate-x-1/2 md:top-1/2 md:-translate-y-1/2">
          {tiles.map((tile) => (
            <img
  ...
  if (firstEmbed) {
    return (
      <iframe
        title={`${first?.title || "Konum"} harita onizlemesi`}
        src={firstEmbed}
        loading="lazy"
  ...
  return (
    <div className="grid h-36 place-items-center bg-[radial-gradient(circle_at_center,rgba(214,255,0,0.13),transparent_55%),linear-gradient(135deg,#171b20,#090b0d)] md:h-44">
      <div className="text-center">
        <MapPin className="mx-auto mb-1 h-6 w-6" style={{ color: accent }} />
        <p className="text-xs font-semibold" style={{ color: textColor }}>Harita konumu</p>
      </div>
    </div>
  );
}
...
type PublicBioPageProps = {
  slug?: string;
};

export default function PublicBioPage({ slug: slugProp }: PublicBioPageProps = {}) {
  ...
  const shareNative = async () => {
    ...
    if (navigator.share) {
      await navigator.share({
        ...
        text: "llinktr ile sen de payla_",
        url: pageUrl,
      });
      return;
    }
  ...
  if (isPaused) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#05070d] text-white px-4">
        ...
        <h1 className="text-2xl font-bold mb-2">Bu sayfa _uanl1k tatil modunda</h1>
        <p className="text-gray-300 mb-6 text-center">Sayfa sahibi yay1n1 ge�ici olarak durdurdu. L�tfen daha sonra tekrar ziyaret edin.</p>
        <Link href="/">
  ...
  return (
    <div
      className="public-bio-shell flex min-h-svh flex-col items-center justify-start overflow-x-hidden px-4 pb-3 pt-4 md:px-4 md:pb-4 md:pt-6"
      style={{
        ...pageBackgroundStyle,
    ...
    >
      <main
        className="public-bio-card mx-auto flex w-full max-w-[420px] flex-col items-center overflow-x-hidden rounded-[20px] border px-6 pb-5 pt-6 text-center md:max-w-[35rem] md:rounded-[2.35rem] md:p-8 lg:max-w-[37rem]"
        style={{
          ...cardStyle,
      ...
      >
        ...
        <div className="w-full space-y-3.5 md:space-y-4">
          {contentBlocks.map((block) => {
            ...
            if (blockType === "divider") {
              ...
              const dividerColor = getDividerColor(blockData, themeConfig.cardBorder);
              return (
                <hr
                  key={block.id}
            ...
            if (blockType === "location") {
              const locations = getLocationEntries(blockData);
              if (locations.length === 0) return null;
              const firstLocation = locations[0];
              const blockTitle = getLocationBlockTitle(blockData, locations);
              const blockDescription = getLocationBlockDescription(blockData);

              return (
                <div
                  key={block.id}
                  className="mx-auto w-full max-w-[21rem] overflow-hidden rounded-[1.35rem] border text-left md:max-w-none"
                  style={{ background: themeConfig.cardBg, borderColor: themeConfig.cardBorder, boxShadow: "0 18px 48px rgba(0,0,0,0.28)" }}
                >
                  <div className="border-b border-white/10 p-4 md:p-5">
                    <div className="flex items-start gap-3">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-black/20 shadow-[0_10px_24px_rgba(0,0,0,0.24)]" style={{ background: getMarkerColor(firstLocation.markerColor) }}>
                        <MapPin className="h-5 w-5 text-black" />
                      </span>
                      <div className="min-w-0 flex-1">
                        <div className="flex min-w-0 flex-wrap items-center gap-2">
                          <p className="line-clamp-2 text-base font-black leading-tight md:text-lg" style={{ color: resolvedTextColor }}>{blockTitle}</p>
                          {locations.length > 1 ? (
                            <span className="rounded-full border px-2.5 py-1 text-[10px] font-bold" style={{ borderColor: themeConfig.cardBorder, color: resolvedTextColor }}>
                        ...
                        {blockDescription ? <p className="mt-2 text-xs leading-relaxed opacity-75 md:text-sm" style={{ color: resolvedTextColor }}>{blockDescription}</p> : null}
                        {firstLocation.address ? <p className="mt-2 line-clamp-2 text-[11px] leading-relaxed opacity-60" style={{ color: resolvedTextColor }}>{firstLocation.address}</p> : null}
                      </div>
                    </div>
                  </div>
                  <LocationMapView entries={locations} accent={accent} textColor={resolvedTextColor} />
                  <div className="space-y-2.5 p-3.5 md:p-4">
                    {locations.map((location, index) => {
                      const entryData = locationEntryToData(location);
                      const directionsUrl = buildMapDirectionsUrl(entryData);
                      const provider = location.provider === "auto_maps" && location.url ? detectMapProvider(location.url) : location.provider;
                      const href = directionsUrl ? (locations.length === 1 ? `/go/${block.id}` : directionsUrl) : "#";

                      return (
                        <div key={location.id} className="rounded-2xl border border-white/10 bg-black/15 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                          <div className="mb-2.5 flex items-start gap-3">
                            <span className="mt-1 h-3.5 w-3.5 shrink-0 rounded-full ring-4 ring-white/5" style={{ background: getMarkerColor(location.markerColor) }} />
                            <div className="min-w-0 flex-1 space-y-1">
                              <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                                <p className="truncate text-sm font-extrabold" style={{ color: resolvedTextColor }}>{location.title || `Konum ${index + 1}`}</p>
                                {provider !== "auto_maps" ? (
                                  <span className="rounded-full border px-1.5 py-0.5 text-[9px] font-semibold opacity-80" style={{ borderColor: themeConfig.cardBorder, color: resolvedTextColor }}>
                                    {getLocationProviderLabel(provider)}
                                  </span>
                                ) : null}
                              </div>
                              {location.address ? <p className="line-clamp-2 text-[11px] leading-relaxed opacity-65" style={{ color: resolvedTextColor }}>{location.address}</p> : null}
                              {!getValidCoordinates(entryData) ? (
                                <p className="mt-1 text-[10px] font-semibold text-amber-300">Bu konumun koordinati eksik; haritada marker gosterilemiyor.</p>
                              ) : null}
                            </div>
                          </div>
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="grid min-h-11 place-items-center rounded-xl border px-4 py-2.5 text-center text-sm font-black tracking-[0.01em] shadow-[0_0_18px_rgba(204,255,0,0.38)] transition hover:-translate-y-0.5 hover:brightness-110 hover:border-white/90 hover:shadow-[0_0_26px_rgba(204,255,0,0.56)] active:translate-y-0 active:scale-[0.98]"
                            style={{ background: "#ccff00", color: "#050505", borderColor: "rgba(255,255,255,0.55)" }}
                          >
                            {location.buttonText || "Yol Tarifi Al"}
                          </a>
                        </div>
                      );
                    })}
        ...
        <div className="mt-6 flex items-center justify-center gap-2 md:mt-8" style={{ color: resolvedTextColor }}>
          <Zap className="h-3.5 w-3.5" />
          <Link href="/" className="text-xs font-medium hover:opacity-70 transition-opacity">
            llinktr ile sen de payla_
          </Link>
        </div>
        <div className="mt-2 text-center">
          <Link href="/report" className="text-[11px] font-medium opacity-55 transition-opacity hover:opacity-90" style={{ color: resolvedTextColor }}>
            0�erii raporla
          </Link>
        </div>
      </main>
          ...
          <div className="w-full max-w-md rounded-[1.6rem] bg-white p-5 text-slate-900 shadow-2xl">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold">Payla_</h2>
              </div>
              <button
                type="button"
