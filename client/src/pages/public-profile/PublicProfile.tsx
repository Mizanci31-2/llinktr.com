...
export default function PublicProfile({ username: usernameProp }: PublicProfileProps = {}) {
  const params = useParams<{ username: string }>();
  const username = useMemo(() => usernameProp || params.username || "", [params.username, usernameProp]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-8">
      <div className="mx-auto max-w-md">
        ...
        <div className="mb-8 space-y-3">
          {links.length > 0 ? (
            ...
            <Card className="p-8 text-center">
              <p className="text-slate-600">Hen�z link eklenmemi_.</p>
            </Card>
          )}
        </div>
        ...
        <div className="mt-8 text-center text-xs text-slate-500">
          ...
          <a href="/report" className="mt-2 inline-block font-medium text-slate-400 hover:text-slate-700">
            0�erii raporla
          </a>
        </div>
      </div>
